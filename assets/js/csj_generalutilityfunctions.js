...
function calculateDistanceSortSlice(body, foundLat, foundLng) {
  ...
  for (i = 0; i < body.features.length; i++) {
    valueList.push({
      ...
      CityCouncilDistrict: fixIfNull(
        body.features[i].attributes.CITYCOUNCILDISTRICT
      ),
      LastUpdated: convertUnixTime(
        fixIfNull(body.features[i].attributes.LASTUPDATE)
      ),
      ...
      Distance: calculateDistance(
        foundLat,
        foundLng,
        body.features[i].geometry.y,
        body.features[i].geometry.x
      ).toFixed(2),
    });
  }
...
function toTitleCase(str) {
  //makes a string become proper case
